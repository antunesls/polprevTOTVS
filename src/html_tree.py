...
function toggleTreeGroup(el){{var a=el.querySelector('.ar');a.classList.toggle('open');el.nextElementSibling.classList.toggle('open')}}
function renderTree(){{
  let q=(document.getElementById('ts').value||'').toLowerCase(),grp={{}};
  rules.forEach(function(r,i){{if(q&&!r.rule_name.toLowerCase().includes(q)&&!r.routines.some(function(rt){{return rt.routine.toLowerCase().includes(q)||(rt.description||'').toLowerCase().includes(q)}}))return;let g=gtg(r.tier);grp[g]=grp[g]||[];grp[g].push({{...r,_i:i}})}});
  let h='';Object.keys(grp).forEach(function(n){{let it=grp[n];let d={{MANTER:'dot-ok',COMPLEMENTAR:'dot-part',CRIAR:'dot-new'}};h+='<div class="tg"><div class="tgh" onclick="toggleTreeGroup(this)"><span class="ar open">'+String.fromCharCode(9654)+'</span>'+esc(n)+'<span class="tgc">'+it.length+'</span></div><div class="tgi open">';it.forEach(function(r){{h+='<div class="ti'+(selIdx===r._i?' sel':'')+'" onclick="select('+r._i+')"><span class="dot '+(d[r.action]||'dot-ok')+'"></span>'+esc(r.rule_name)+(r._marked_for_delete?' <span style="color:#ef4444;font-size:8px">DEL</span>':'')+'</div>'}});h+='</div></div>'}});document.getElementById('treeBody').innerHTML=h;
}}
...
function renderDetail(){{
  ...
  let r=rules[selIdx];
  let mis=0;r.routines.forEach(function(rt){{rt.features.forEach(function(f){{if(f.status==='FALTANTE')mis++}})}});
  let exc=0;r.routines.forEach(function(rt){{rt.features.forEach(function(f){{if(f.status==='EXCEDENTE')exc++}})}});
  let bd=r.action==='MANTER'?'db-ok':r.action==='COMPLEMENTAR'?'db-part':'db-new';
  let h='';
  h+='<div class="dh"><div><div class="dt">'+esc(r.rule_name)+'</div><div class="ds">'+esc(r.tier)+' | '+(r.rule_description||'')+'</div></div><span class="db '+bd+'">'+(''+(r._marked_for_delete?'REMOVIDO':r.action))+'</span></div>';
  if(exc>0)h+='<div class="missing-box" style="border-color:#3b0764"><h4 style="color:#c084fc">EXCEDENTE: '+exc+' features</h4></div>';
  if(mis>0){{h+='<div class="missing-box"><h4>'+mis+' complementos necessarios</h4>';r.routines.forEach(function(rt){{rt.features.forEach(function(f){{if(f.status==='FALTANTE')h+='<div>'+esc(rt.routine)+' -> '+esc(f.feature||'')+'</div>'}})}})}};h+='</div>'
  if(mis===0&&r.action==='MANTER')h+='<div class="missing-box" style="border-color:#14532d"><h4 style="color:#22c55e">OK Completo</h4></div>';
  h+='<div class="sec"><h3>Rotinas ('+r.routines.length+')</h3>';r.routines.forEach(function(rt){{h+='<div class="rt-row"><span class="rt-code">'+esc(rt.routine)+'</span><span class="rt-desc">'+esc(rt.description||'')+'</span><div class="ft-list">';rt.features.forEach(function(f){{h+='<span class="ft ft-'+(f.status==='EXISTENTE'?'ok':f.status==='FALTANTE'?'mis':'exc')+'">'+esc(f.feature||'')+'</span>'}});h+='</div></div>'}});h+='</div>';
  h+='<div class="sec"><h3>Usuarios ('+r.users.length+')</h3>';r.users.forEach(function(u){{var lidx=selIdx;h+='<div class="user-row"><span class="user-chip">'+esc(u.login||u.user_id)+' <button onclick="rmUsr('+lidx+','+q(u.login||u.user_id)+')">x</button></span></div>'}});h+='<div class="user-row"><input id="nu" placeholder="login" style="background:#1a1a3a;border:1px solid #2a2a4a;border-radius:4px;padding:3px 6px;color:#d4d4e0;font-size:10px;width:120px"><button class="btn btn-b" style="padding:3px 8px;font-size:9px" onclick="addUsr('+selIdx+')">+</button></div></div>';
  h+='<div style="display:flex;gap:8px;margin-top:16px"><button class="btn btn-r" onclick="delRule('+selIdx+')">'+(r._marked_for_delete?'Desfazer':'Remover')+'</button> <button class="btn btn-b" onclick="refreshSQL()">Atualizar SQL</button> <button class="btn btn-s" onclick="editName('+selIdx+')">Renomear</button></div>';
