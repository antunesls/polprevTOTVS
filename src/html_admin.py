...
<button class="btn btn-r" onclick="bulkMarkSelected(true)">Remover Selecionadas</button>
<button class="btn btn-s" onclick="bulkMarkSelected(false)">Desfazer Selecionadas</button>
<button class="btn btn-b" onclick="addRule()">+ Nova Regra</button>
<button class="btn btn-g" onclick="genSQL()">⚡ Gerar SQL</button>
<button class="btn btn-g" onclick="genSQLForUser(document.getElementById('fUser').value)">👤 SQL por Usuário</button>
...
<select id="fUser" onchange="render()" style="max-width:180px"><option value="">Usuário...</option></select>
...
function init(){{
  ...
  let userSet=new Set();
  rules.forEach(r=>(r.users||[]).forEach(u=>{{let k=u.login||u.user_id;if(k)userSet.add(k)}}));
  let userList=[...userSet].sort();
  document.getElementById('fUser').innerHTML='<option value="">Todos usuarios</option>'+userList.map(u=>'<option value="'+esc(u)+'">'+esc(u)+'</option>').join('');
  refresh();
}}
...
function render(){{
  let fd=rules.filter(r=>{{let a=document.getElementById('fAction').value,t=document.getElementById('fTier').value,s=document.getElementById('fSource').value,q=(document.getElementById('fSearch').value||'').toLowerCase(),fu=document.getElementById('fUser').value;if(a&&r.action!==a)return false;if(t&&r.tier!==t)return false;if(s&&r.source!==s)return false;if(fu&&!(r.users||[]).some(u=>(u.login||u.user_id)===fu))return false;if(q&&!r.rule_name.toLowerCase().includes(q)&&!r.routines.some(rt=>rt.routine.toLowerCase().includes(q)))return false;return true}});
...
function bulkMarkSelected(marked){{let ids=[...selected];if(!ids.length){{toast('Nenhuma regra selecionada');return}}if(marked&&!confirm('Marcar '+ids.length+' regras para remocao de vinculos?'))return;ids.forEach(i=>{{if(rules[i])rules[i]._marked_for_delete=marked}});selected.clear();render();toast(marked?'Regras marcadas para remocao':'Remocao desfeita')}}
...
function openMergeModal(ti){{let candidates=[];for(let i=0;i<rules.length;i++){{if(i===ti)continue;if(rules[i]._marked_for_delete)continue;if(!rules[i].routines.length)continue;candidates.push({{idx:i,name:rules[i].rule_name,routines:rules[i].routines.length,action:rules[i].action,tier:rules[i].tier}})}}if(!candidates.length){{toast('Nenhum outro conjunto disponivel para incorporar');return}}let h='';h+='<h2>Incorporar conjunto em '+esc(rules[ti].rule_name)+'</h2>';h+='<p style="color:#94a3b8;margin-bottom:12px;font-size:11px">A origem sera marcada para remocao de vinculos apos a incorporacao.</p>';h+='<div class="sec"><h3>Origem ('+candidates.length+' candidatos)</h3>';candidates.forEach(function(c){{h+='<div class="row" style="justify-content:space-between"><span style="font-size:11px"><strong>'+esc(c.name)+'</strong> <span style="color:#64748b">'+esc(c.tier)+'</span> — '+c.routines+' rotinas — <span style="color:#eab308">'+esc(c.action)+'</span></span><button class="btn btn-b" style="font-size:10px;padding:3px 10px" onclick="mergeRuleInto('+ti+','+c.idx+')">'+String.fromCharCode(9654)+' Incorporar</button></div>'}});h+='</div><div style="margin-top:12px;text-align:right"><button class="btn btn-s" onclick="closeModal()">Cancelar</button></div>';document.getElementById('mc').innerHTML=h;document.getElementById('mo').classList.add('show')}}
function mergeRuleInto(ti,si){{let target=rules[ti];let source=rules[si];if(!confirm('Incorporar '+source.rule_name+' em '+target.rule_name+'?\\n\\n'+source.rule_name+' sera marcado para remocao de vinculos.'))return;let targetRtMap={{}};target.routines.forEach(function(rt){{targetRtMap[rt.routine]=rt}});source.routines.forEach(function(sr){{let tr=targetRtMap[sr.routine];if(!tr){{let copy=JSON.parse(JSON.stringify(sr));copy.features.forEach(function(f){{f.status='FALTANTE'}});target.routines.push(copy);targetRtMap[sr.routine]=copy}}else{{sr.features.forEach(function(sf){{let exists=tr.features.some(function(tf){{return tf.feature===sf.feature}});if(!exists){{let copy={{...sf,status:'FALTANTE'}};tr.features.push(copy)}}}})}}}});if(target.source==='EXISTENTE'&&target.action==='MANTER')target.action='COMPLEMENTAR';source._marked_for_delete=true;closeModal();render();toast('Incorporado: '+source.rule_name+' '+String.fromCharCode(8594)+' '+target.rule_name+' ('+source.routines.length+' rotinas)')}}
...
function genSQLForUser(login){{if(!login){{toast('Selecione um usuario primeiro');return}}let involved=rules.filter(r=>!r._marked_for_delete&&r.action!=='MANTER'&&(r.users||[]).some(u=>(u.login||u.user_id)===login));if(!involved.length){{toast('Nenhuma regra pendente para '+login);return}}let userEntry=null;for(let r of involved){{for(let u of(r.users||[])){{if((u.login||u.user_id)===login){{userEntry=u;break}}}}if(userEntry)break}}let userId=userEntry?userEntry.user_id||login:login;let ls=[];ls.push('-- ==============================================');ls.push('-- DELTA SQL — Usuario: '+login+' (ID: '+userId+')');ls.push('-- '+new Date().toISOString().slice(0,10));ls.push('-- Regras envolvidas: '+involved.length);ls.push('-- ==============================================');ls.push('');ls.push('BEGIN TRANSACTION');ls.push('');let s=1;involved.forEach(r=>{{let rid=r.rule_id||('A'+String(s++).padStart(5,'0'));if(r.source==='NOVO'){{ls.push('-- NOVA REGRA: '+r.rule_name);ls.push('INSERT INTO SYS_RULES (RL__ID, RL__CODIGO) VALUES ('+q(rid)+', '+q(r.rule_name)+');');ls.push('')}}else{{ls.push('-- '+r.rule_name+' — complementos (usa RL__ID '+q(rid)+')');ls.push('')}}r.routines.forEach(rt=>rt.features.filter(f=>f.status==='FALTANTE').forEach(f=>ls.push('INSERT INTO SYS_RULES_FEATURES (RL__ID, RL__ROTINA, RL__DESMDEF, RL__ACESSO) VALUES ('+q(rid)+', '+q(rt.routine)+', '+q(f.feature||'')+', '+q(f.access||'1')+');')));if(r.source==='NOVO')r.routines.forEach(rt=>ls.push('INSERT INTO SYS_RULES_TRANSACT (RL__ID, RL__ROTINA, RL__DESROT, RL__ACESSO, RL__CHKSUM, D_E_L_E_T_) VALUES ('+q(rid)+', '+q(rt.routine)+', '+q((rt.description||'').slice(0,40))+', '+q('1')+', '+q('')+', '+q(' ')+');'));ls.push('-- Vinculo: '+login);ls.push('INSERT INTO SYS_RULES_USR_RULES (USER_ID, USR_RL_ID) VALUES ('+q(userId)+', '+q(rid)+');');ls.push('')}});ls.push('COMMIT');sqlCache=ls.join(String.fromCharCode(10));toast('SQL gerado para '+login+': '+involved.length+' regras')}}
